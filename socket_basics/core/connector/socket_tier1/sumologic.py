"""SumoLogic notifier formatting for Socket Tier1 reachability analysis."""
...
def format_notifications(components_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for comp in components_list:
        ...
        for a in comp.get('alerts', []):
            ...
            rows.append([
                ...
                f"scanner=socket-tier1"
            ])
    
    # Format as structured data for SumoLogic
    if not rows:
        content = "No Socket Tier1 vulnerabilities found."
    else:
        headers = ['Vulnerability', 'Severity', 'Reachability', 'Package', 'Component', 'Scanner']
    ...
    return [{
        'title': 'Socket Tier1 Vulnerability Events',
        'content': content
    }]