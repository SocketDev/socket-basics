"""
Microsoft Teams text formatter for Socket Security Basics.

This module provides text formatting for Microsoft Teams notifications, 
converting security findings into readable text format suitable for Teams messages.
Note: This converts findings to text rather than MessageCard format.
"""

from typing import Any, List
from .base import TableFormatter

# Teams limits (similar to Slack)
MAX_TEAMS_MESSAGE_SIZE = 28000  # Conservative limit for Teams messages


class TeamsFormatter(TableFormatter):
    """Formatter for Microsoft Teams text messages."""
    
    def __init__(self):
        super().__init__(max_content_length=MAX_TEAMS_MESSAGE_SIZE)
    
    def format_findings(self, findings: List[dict], title: str = "Security Findings") -> str:
        """Format security findings as Teams-friendly text.
        ...
        Returns:
            Formatted text suitable for Teams
        """
        if not findings:
        ...
        for i, finding in enumerate(findings, 1):
            ...
            output_lines.append(f"{i}. {severity_icon} **{severity.upper()}** - {component}")
    ...
    def format_table(self, headers: List[str], rows: List[List[Any]], title: str = "Results") -> str:
        """Format tabular data as Teams-friendly text.
