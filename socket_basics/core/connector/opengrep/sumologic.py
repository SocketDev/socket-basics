#!/usr/bin/env python3
"""
SumoLogic notifier for OpenGrep results.
Formats results in structured logging format suitable for log parsing.
"""
...
def format_notifications(groups: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    """Format for SumoLogic - return multiple structured log datasets grouped by subtype."""
    tables = []
    ...
    for subtype, items in groups.items():
        ...
        for item in items:
            ...
            rows.append((
                ...
                [
                    f"rule={props.get('ruleId', a.get('title', ''))}",
                    f"severity={a.get('severity', '')}",
                    f"file={props.get('filePath', '')}",
        ...
        # Apply truncation
        result_limit = _get_sumologic_result_limit()
        total_results = len(rows)
        was_truncated = False
        
        if total_results > result_limit:
            logger.info(f"Truncating SumoLogic OpenGrep results from {total_results} to {result_limit} (prioritized by severity)")
            rows = rows[:result_limit]
            was_truncated = True
        
        # Create a separate log dataset for each subtype/language group
        display_name = subtype_names.get(subtype, subtype.upper())
        headers = ['Rule', 'Severity', 'File', 'Lines', 'Type', 'Scanner']
        ...
        if was_truncated:
            content += f"\n\nresults_truncated=true total_results={total_results} displayed_results={result_limit} note=\"Results truncated to {result_limit} highest severity findings. See full scan URL for complete results.\""
