...
cd socket-basics
docker build -t socket-basics:1.0.26 .
...
docker build -t socket-basics:1.0.26:latest .
...
docker build -t myorg/security-scanner:1.0.26 .

# Build for specific platform (e.g., for M1/M2 Macs)
docker build --platform linux/amd64 -t socket-basics:1.0.26 .
```
...
docker run --rm socket-basics:1.0.26 socket-basics --version
docker run --rm socket-basics:1.0.26 socket --version
docker run --rm socket-basics:1.0.26 trivy --version
docker run --rm socket-basics:1.0.26 opengrep --version
docker run --rm socket-basics:1.0.26 trufflehog --version
```
...
pipeline {
    ...
    stages {
        stage('Security Scan') {
            steps {
                script {
                    docker.image('socket-basics:1.0.26').inside(
...
alias sb-docker='docker run --rm -v "$PWD:/workspace" --env-file .env socket-basics:1.0.26 --workspace /workspace'
...
# Rebuild image
alias sb-build='docker build -t socket-basics:1.0.26 .'
```
...
IMAGE_NAME="socket-basics:1.0.26"
ENV_FILE=".env"
